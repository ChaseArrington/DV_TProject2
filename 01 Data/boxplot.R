...
ndf <- df %>% select(YEARID,AB,H,HR, BB, HBP, SF, PLAYERID, TEAMID) %>% filter(TEAMID %in% c("TEX","SFN"), YEARID == 2010) %>% mutate(obp = (H + BB + HBP)/(AB + BB + HBP + SF))

filter(cut %in% c("Good", "Fair"), clarity == "VS1") %>% tbl_df

ggplot() + 
  coord_cartesian() + 
  scale_x_discrete() +
  scale_y_discrete() +
  labs(title='OBP Cross Tab') +
  labs(x=paste("TeamID"), y=paste("PlayerID")) +
  layer(data=ndf, 
